<?php
$EMAIL = ''; //  Ваш e-mail на который будут приходить заявки
$FROM = '';  //  e-mail отправителя (иногда требуется указать разрешенный настройками e-mail)
...
switch (@$_POST['action']) { // рассматриваем варианты разных значений поля action
    ...
    break;
    case 'service_order':
        sendmail("ЗАКАЗ ПОДАРКОВ", render(
                        '<div><b>Ваша Компания:</b> '.stripinput($_POST['company']).'</div>'.
                        '<div><b>Телефон:</b> '.stripinput($_POST['tel']).'</div>'.
                        '<div><b>Количество товаров:</b> '.stripinput($_POST['list']).'</div>'.
                        '<div><b>Название товара:</b> '.stripinput($_POST['artist']).'</div>'.
